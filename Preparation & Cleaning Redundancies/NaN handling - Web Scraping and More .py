...
# OR upload from local path
    # local_file = 'insert your path here\\premises-licence-register.csv'
...
if ans == 'y':
    ...
    LA_filled.to_csv('Insert Path - Key Stage Downloads - CSVs\\Local Authority Filled.csv', index=True) # change location
    print('success', LA_filled)
elif ans == 'n':
    la_filled_df = pd.read_csv('Insert Path - Key Stage Downloads - CSVs\\Local Authority Filled.csv')
...
df_local.to_csv('Insert Path - Key Stage Downloads - CSVs\\NONULLS_premises-licence-register.csv')
